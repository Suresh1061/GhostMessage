...
const Page = () => {
  ...
  const initialSuggesMessages = "What's your favorite movie?||Do you have any pets?||What's your dream job?";
  const seperator = "||"
      ...
      <div className="w-full space-y-4 mt-12">
        ...
        <div className="rounded-lg  bg-card text-card-foreground sm:border sm:shadow-sm px-2 sm:px-6 pt-6">
          ...
          <div className="space-y-2 py-6">
            {text ? splitMessages(text).map((message, index) => (
              <SuggestMessageBtn
                key={index}
                message={message}
              ...
              splitMessages(initialSuggesMessages).map((message, index) => (
                <SuggestMessageBtn
                  key={index}
                  message={message}
                  setValue={() => form.setValue('content', message)}
